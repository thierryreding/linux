...
static int tegra_xusb_load_firmware(struct tegra_xusb *tegra)
{
	...
	major = (header->version_id >> 24) & 0xff;
	minor = (header->version_id >> 16) & 0xff;

	timestamp = le32_to_cpu(header->fwimg_created_time);
	time64_to_tm(timestamp, 0, &time);

	dev_info(dev, "Firmware: %x.%02x (%ld-%02d-%02d %02d:%02d:%02d UTC)\n",
		 major, minor, time.tm_year + 1900, time.tm_mon + 1,
